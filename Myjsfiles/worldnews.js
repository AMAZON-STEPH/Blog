...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("https://newsapi-w6iw.onrender.com/api/news/category/world")
    .then((fullfilled) => fullfilled.json())
    .then((content) => {
      const worldnewsContainer = document.getElementById("worldnewscontainer");
  ...
  `;
         ...
         items.addEventListener("click", () => {
          const slug = items.getAttribute("data-slug");
            if (slug) {
            window.location.href = `../detail/worldnewsdetail.html?id=${slug}`;
            }
          });
          worldnewsContainer.appendChild(items);
          console.log(items);
        });
      };
      ...
      setInterval(() => {
        fade(worldnewsContainer, "out");
        setTimeout(() => {
          display = display === 0 ? 5 : 0;
          rendernews(display);
          fade(worldnewsContainer, "in");
        }, 600);
      }, 8000);
    })
